...
pub type TimerDuration<T, const FREQ_HZ: u64> = Duration<T, { Fraction::new(1, FREQ_HZ) }>;

/// Alias for durations that come from timers with a specific frequency (`u32` backing storage)
pub type TimerDurationU32<const FREQ_HZ: u64> = Duration<u32, { Fraction::new(1, FREQ_HZ) }>;

/// Alias for durations that come from timers with a specific frequency (`u64` backing storage)
pub type TimerDurationU64<const FREQ_HZ: u64> = Duration<u64, { Fraction::new(1, FREQ_HZ) }>;

// -------------------------------

/// Alias for instants that come from timers with a specific frequency
pub type TimerInstant<T, const FREQ_HZ: u64> = Instant<T, { Fraction::new(1, FREQ_HZ) }>;

/// Alias for instants that come from timers with a specific frequency (`u32` backing storage)
pub type TimerInstantU32<const FREQ_HZ: u64> = Instant<u32, { Fraction::new(1, FREQ_HZ) }>;

/// Alias for instants that come from timers with a specific frequency (`u64` backing storage)
pub type TimerInstantU64<const FREQ_HZ: u64> = Instant<u64, { Fraction::new(1, FREQ_HZ) }>;
...
pub type TimerRate<T, const FREQ_HZ: u64> = Rate<T, { Fraction::new(FREQ_HZ, 1) }>;

/// Alias for rate that come from timers with a specific frequency (`u32` backing storage)
pub type TimerRateU32<const FREQ_HZ: u64> = Rate<u32, { Fraction::new(FREQ_HZ, 1) }>;

/// Alias for rate that come from timers with a specific frequency (`u64` backing storage)
pub type TimerRateU64<const FREQ_HZ: u64> = Rate<u64, { Fraction::new(FREQ_HZ, 1) }>;
