...
macro_rules! impl_rate_for_integer {
    ($i:ty) => {
        impl<const F: Fraction> Rate<$i, F> {
            ...
            /// ```
            /// # use fugit::*;
            #[doc = concat!("let r1 = Rate::<", stringify!($i), ", { Fraction::new(1, 1_000) }>::from_raw(10);")]
            #[doc = concat!("let r2 = Rate::<", stringify!($i), ", { Fraction::new(1, 1_000) }>::from_raw(3);")]
            ///
            ...
            pub const fn const_try_from<const I: Fraction>(
                ...
                if Helpers::<I, F>::SAME_BASE {
                        ...
                        .checked_mul(Helpers::<I, F>::RD_TIMES_LN)
                    {
                        ...
                        let raw = (lh + Helpers::<I, F>::LD_TIMES_RN / 2)
            ...
            #[doc = concat!("let d1: Option<Duration::<", stringify!($i), ", { Fraction::new(1, 1_000) }>> = r1.try_to_duration();")]
            ///
            /// assert_eq!(d1.unwrap().as_ticks(), 1_000);
            /// ```
            pub const fn try_to_duration<const O: Fraction>(
                self,
            ) -> Option<Duration<$i, O>> {
            ...
            #[track_caller]
            pub const fn to_duration<const O: Fraction>(
                self,
            ) -> Duration<$i, O> {
            ...
            pub const fn to_Hz(&self) -> $i {
                    (Helpers::<{ Fraction::ONE }, F>::LD_TIMES_RN as $i * self.raw
                        + Helpers::<{ Fraction::ONE }, F>::RD_TIMES_LN as $i / 2)
            ...
            pub const fn to_kHz(&self) -> $i {
                    (Helpers::<{ Fraction::KILO }, F>::LD_TIMES_RN as $i * self.raw
                        + Helpers::<{ Fraction::KILO }, F>::RD_TIMES_LN as $i / 2)
            ...
            pub const fn to_MHz(&self) -> $i {
                    (Helpers::<{ Fraction::MEGA }, F>::LD_TIMES_RN as $i * self.raw
                        + Helpers::<{ Fraction::MEGA }, F>::RD_TIMES_LN as $i / 2)
        ...
        // Rate % Rate = Rate (only same base until const_generics_defaults is
        // stabilized)
        impl<const F: Fraction> ops::Rem for Rate<$i, F>
        {
            type Output = Self;
        ...
        // Rate %= Rate
        impl<const F: Fraction> ops::RemAssign for Rate<$i, F>
        {
            #[inline]
...
impl<const F: Fraction> From<Rate<u32, F>> for Rate<u64, F> {
    ...
    fn from(val: Rate<u32, F>) -> Rate<u64, F> {
        Rate::<u64, F>::from_raw(val.to_raw() as u64)
    }
}
...
impl<const F: Fraction> ops::Sub<Rate<u32, F>> for Rate<u64, F> {
    ...
    fn sub(self, other: Rate<u32, F>) -> Self::Output {
        if let Some(v) = self.checked_sub(Self::from_raw(other.to_raw() as u64)) {
            v
        } else {
            panic!("Sub failed!");
        }
...
impl<const F: Fraction> ops::Add<Rate<u32, F>> for Rate<u64, F> {
    ...
    fn add(self, other: Rate<u32, F>) -> Self::Output {
        if let Some(v) = self.checked_add(Self::from_raw(other.to_raw() as u64)) {
            v
        } else {
            panic!("Add failed!");
        }
...
impl<const L: Fraction, const R: Fraction> PartialOrd<Rate<u32, R>> for Rate<u64, L> {
    ...
    fn partial_cmp(&self, other: &Rate<u32, R>) -> Option<Ordering> {
        self.partial_cmp(&Rate::<u64, R>::from_raw(other.to_raw() as u64))
    }
}
...
impl<const L: Fraction, const R: Fraction> PartialEq<Rate<u32, R>> for Rate<u64, L> {
    ...
    fn eq(&self, other: &Rate<u32, R>) -> bool {
        self.eq(&Rate::<u64, R>::from_raw(other.to_raw() as u64))
    }
}
